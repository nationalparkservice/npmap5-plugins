    ...
    return class ArcGisRest extends mapLibrary['GeoJSONSource'] implements Source {
        ...
        async loadMapData(map?: MapElement, bounds?: [number, number, number, number]) {
            ...
            if (this.promoteId) {
                ...
                const alreadyLoadedKeys = await (this._geometriesAtZoom.exec('getKeysAtZoom') as GeometriesAtZoom['getKeysAtZoom'])(displayZoom);
        ...
        async drawMapData(newEsriJson: esriFeatureServiceResponse | esriMapServiceResponse, displayZoom: number) {
            ...
            const dataDiff = {
                ...
                add: (Feature<Geometry, GeoJsonProperties>)[];
                update: ({
                    id: string;
            ...
            if (dataDiff.update.length || dataDiff.add.length) {
                ...
                if ((this as any).updateData && false) {
                    // TODO Maplibre version 3!
                    // This is untested, so don't enable it!
                    (this as any).updateData(dataDiff);
                } else {
                    // Update the _data in place
                    const currentFeatures = (this._data as FeatureCollection).features;
                    const currentFeaturesIds = currentFeatures.map(feature => (feature.properties as any)[this.promoteId as string] as string);
